...
import __APP_DATA__ from './api/data';

/**
 * Ink web component class
 */
export default abstract class ClientComponent extends HTMLElement {
  //ie. abc123
  ...
  protected _definition: ClientComponentClass|null = null;
  ...
  public get definition() {
    return this._definition || this.constructor as ClientComponentClass;
  }

  /**
   * Returns the component's element registry
   */
  public get element(): ClientElement {
    //check to see if the registry has this component
    if (!ClientRegistry.has(this)) {
      //it should have...
      throw new Error(`Component ${this.metadata.classname} not mapped.`);
    }
    return ClientRegistry.get(this) as ClientElement;
  }
   ...
   */
  public set definition(definition: ClientComponentClass) {
  ...
  public constructor() {
    ...
    if (!ClientRegistry.has(this)) {
      ...
      if (!registered) {
        ...
        } not registered in customElements.`);
      }
      //get all attributes
      ...
      ClientRegistry.register(this, attributes);
    }
  }
  ...
  public cloneElement(element: Element, andChildren = false) {
    return ClientRegistry.cloneElement(element, andChildren);
  }
  ...
  public getElement(element: Element) {
    return ClientRegistry.get(element);
  }
  ...
  public getParentComponent() {
    ...
    while (parent) {
      if (parent instanceof ClientComponent) {
        return parent;
      }
  ...
  public register(attributes: Hash = {}, children: AnyChild[] = []) {
    ...
    if (ClientRegistry.has(this)) {
      ...
      const element = ClientRegistry.get(this) as ClientElement;
  ...
  protected _cleanChildren(children: AnyChild[]) {
    return Array
      ...
      .map(child => typeof child === 'string' 
        ? ClientRegistry.createText(child) 
  ...
  protected _toNodeList(value: any) {
    ...
    return [ ClientRegistry.createText(String(value)) ];
  }
